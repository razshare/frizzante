...
func TestJavaScriptRun(test *testing.T) {
	// Simple.
	script := "1+1"
	actual, destroy, javaScriptError := libjs.JavaScriptRun("test", []byte(script), map[string]v8go.FunctionCallback{})
	defer destroy()
...
func TestJavaScriptBundle(test *testing.T) {
	...
	cjs, bundleError := libjs.JavaScriptBundle("app", api.FormatCommonJS, []byte(script))
	if bundleError != nil {
		test.Fatal(bundleError)
	}
	actual := ""
	expected := "hello"
	_, destroy, javaScriptError := libjs.JavaScriptRun("test", cjs, map[string]v8go.FunctionCallback{
