...
func Migrate(options MigrateOptions) (err error) {
	...
	if fileNames, err = files.ReadDirectory("migrate"); err != nil {
		return
	}
	for _, fileName := range fileNames {
		if strings.HasSuffix(fileName, ".go") {
			...
			args = append(args, fmt.Sprintf(".%smigrate", string(filepath.Separator)))
			...
			messages.Success("project database migrated")
			break
		}
	}
	return
