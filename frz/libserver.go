...
// WithEfs sets the embedded file system, which
// should usually contain `app/dist`.
func (server *Server) WithEfs(efs embed.FS) *Server {
	server.efs = efs
	return server
}

// WithPublicRoot sets the public root directory,
...
// WithViewRoot sets the view root directory, which is usually `app`.
//
// The view root directory should contain "node_modules" a "package.json".
func (server *Server) WithViewRoot(directoryName string) *Server {
	server.viewRoot = directoryName
	return server
}

// WithViewServer sets the view server,
// which is usually `app/dist/server.js`.
//
// The view server is the JavaScript program that renders
// exported views on the server.
func (server *Server) WithViewServer(fileName string) *Server {
	server.viewServer = fileName
	return server
}

// WithViewIndex sets the view index,
// which is usually `app/dist/client/index.html`.
//
// The view index is the base html document upon which the view
