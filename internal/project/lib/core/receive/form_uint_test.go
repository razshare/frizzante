...
func TestFormUint(t *testing.T) {
	...
	if ok := FormUint(client, "key", &value); !ok || value != 5 {
		t.Fatal("key should be a valid int with value 5")
	}

	if ok := FormUint32(client, "key", &value32); !ok || value32 != 5 {
		t.Fatal("key should be a valid int32 with value 5")
	}

	if ok := FormUint64(client, "key", &value64); !ok || value64 != 5 {
	...
	if ok := FormUint(client, "key", &value); ok || value != 0 {
		t.Fatal("key should not be a valid uint")
	}

	if ok := FormUint32(client, "key", &value32); ok || value32 != 0 {
		t.Fatal("key should not be a valid uint32")
	}

	if ok := FormUint64(client, "key", &value64); ok || value64 != 0 {
	...
	if ok := FormUint(client, "key", &value); ok || value32 != 0 {
		t.Fatal("key should not be a valid uint")
	}
