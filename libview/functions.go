...
import (
	...
	"github.com/razshare/frizzante/libjs"
	"os"
	"regexp"
	"rogchap.com/v8go"
	"strings"
...
func (view *View) Render(efs embed.FS) (html string, renderError error) {
	...
	if libfs.IsDirectory(view.Root) {
		var serverError error
		server, serverError = libjs.JavaScriptBundle(view.Root, api.FormatCommonJS, readBytes)
		if serverError != nil {
			return "", serverError
		}
	} else {
	...
	_, destroy, javaScriptError := libjs.JavaScriptRun(view.Server, bundle, globals)
